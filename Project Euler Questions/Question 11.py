# This will provide the answer to Question 11
...
def mathThis(vert, horz, diagL, diagR):
    """
    :type       vert:   list
    :param      vert:   The vertical
    :type       horz:   list
    :param      horz:   The horizontal
    :type       diagL:  list
    :param      diagL:  The diaganal l
    :type       diagR:  list
    :param      diagR:  The diaganal r

    :returns:   The highest value contained in all Lists
    :rtype:     int
    """
    counter = 0
    holder = 0
    while(counter != len(vert)):
        if(counter != (len(vert) - 1)):
                if(holder < vert[counter]):
                    holder = vert[counter]
        counter += 1

    counter = 0
    while(counter != len(horz)):
        if(counter != (len(horz) - 1)):
                if(holder < horz[counter]):
                    holder = horz[counter]
        counter += 1

    counter = 0
    while(counter != len(diagL)):
        if(counter != (len(diagL) - 1)):
                if(holder < diagL[counter]):
                    holder = diagL[counter]
        counter += 1

    counter = 0
    while(counter != len(diagR)):
        if(counter != (len(diagR) - 1)):
                if(holder < diagR[counter]):
                    holder = diagR[counter]
        counter += 1
    return holder


answer = mathThis(vertHolder, horzHolder, diagLHolder, diagRHolder)
